...
const { generateHTMLDocument, generateHeader, generateStatsGrid, openInBrowser, setTemplateDir, getTemplateLoader } = require('./templates');
const { createCommitUrl, createBranchUrl } = require('./utils');
const config = require('./config');
const charts = require('./charts');
const { createSampleTemplates } = require('./template-loader');
...
function showHelp() {
    ...
    console.log(chalk.green('  -t, --template-dir <dir>') + '   Custom template directory for report styling');
    console.log(chalk.green('  --init-templates') + '           Create sample template files in current directory');
    console.log(chalk.green('  -h, --help') + '                 Show this help message');
    console.log('');
    console.log(chalk.yellow('Template Customization:'));
    console.log(chalk.gray('  Create custom templates to brand your reports:'));
    console.log(chalk.gray('  1. Run: git-moar snitch --init-templates'));
    console.log(chalk.gray('  2. Edit files in ./templates/ directory'));
    console.log(chalk.gray('  3. Run: git-moar snitch --template-dir ./templates'));
    console.log('');
    ...
    console.log(chalk.gray('  git-moar snitch --template-dir ./my-templates -o branded-report.html'));
    console.log('');
}
...
function generateReport(data) {
    ...
    <div class="export-buttons">
        <button class="export-btn" onclick="exportToCSV('commits-table', '${data.repo.name}-commits.csv')">📥 CSV</button>
        <button class="export-btn" onclick="exportToJSON(${JSON.stringify(exportData).replace(/"/g, '&quot;')}, '${data.repo.name}-report.json')">📥 JSON</button>
    </div>
    ...
    <div class="container">
        ...
        ${config.chartsEnabled() ? charts.generateStreakCard(data.commits) : ''}
        ...
        <div class="section">
            ...
            <div class="charts-grid">
                ${data.users && data.users.length > 1 ? charts.generateContributorPieChart(data.users, 'snitch-contributor-chart') : ''}
                ${charts.generateWeeklyActivityChart(data.commits, 'snitch-weekly-chart')}
            </div>
            ${data.locDetails && data.locDetails.byLanguage ? `
            <div class="charts-grid">
                ${charts.generateLanguageDistributionChart(data.locDetails.byLanguage, 'snitch-lang-chart')}
            </div>
            ` : ''}
            <div class="charts-grid">
                ${charts.generateCommitSizeDistribution(data.commits, 'snitch-size-dist')}
                ${charts.generateTimeOfDayChart(data.commits, 'snitch-time-chart')}
            </div>
            ${charts.generateContributionCalendar(data.commits, 'snitch-calendar')}
            <div class="charts-grid">
                ${charts.generateVelocityChart(data.commits, 'snitch-velocity')}
                ${charts.generateCodeOwnershipMatrix(data.commits, 'snitch-ownership')}
            </div>
            ${data.users && data.users.length > 1 ? charts.generateCollaborationNetwork(data.commits, data.users, 'snitch-collab') : ''}
        </div>
        ` : ''}

        ${config.chartsEnabled() && data.users && data.users.length > 1 ? `
        <div class="section">
            <h2>👥 Contributor Insights</h2>
            <div class="charts-grid">
                ${charts.generateUserActivityTimeline(data.commits, data.users, 'snitch-user-timeline')}
                ${charts.generateUserContributionsComparison(data.users, 'snitch-user-contrib')}
            </div>
            <div class="charts-grid">
                ${charts.generateUserCommitSizeComparison(data.commits, data.users, 'snitch-user-size')}
                ${charts.generateUserPeakActivityTimes(data.commits, data.users, 'snitch-user-peaks')}
            </div>
        </div>
        ` : ''}
        ...
        <div class="section">
            ...
            <div style="background: #334155; border-radius: 12px; padding: 20px;">
                <table id="commits-table" class="data-table">
                    <thead>
                        <tr>
                            <th>Hash</th>
                            <th>Branch</th>
                    ...
                    <tbody>
                        ${data.commits.map(commit => `
                            <tr>
                                <td><a href="${createCommitUrl(data.repo.url, commit.hash)}" class="commit-hash" target="_blank">${commit.hash.substring(0, 7)}</a></td>
                                <td><a href="${createBranchUrl(data.repo.url, commit.branch)}" class="commit-branch" target="_blank">${commit.branch}</a></td>
                                <td>${commit.author}</td>
                                <td>${commit.message}</td>
                                <td data-order="${commit.date.getTime()}">${commit.date.toLocaleString()}</td>
                                <td class="additions" data-order="${commit.additions}">+${commit.additions}</td>
                                <td class="deletions" data-order="${commit.deletions}">-${commit.deletions}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
...
function run(args) {
    ...
    if (options.initTemplates) {
        const templatesDir = createSampleTemplates(process.cwd());
        console.log(chalk.green('✅ Sample templates created!'));
        console.log(chalk.gray(`📁 Templates directory: ${templatesDir}`));
        console.log('');
        console.log(chalk.yellow('Next steps:'));
        console.log(chalk.gray('  1. Edit the template files to customize your reports'));
        console.log(chalk.gray(`  2. Run: git-moar snitch --template-dir ${templatesDir}`));
        return;
    }

    // Set custom template directory if provided
    if (options.templateDir) {
        const resolvedPath = path.resolve(options.templateDir);
        if (!fs.existsSync(resolvedPath)) {
            console.log(chalk.red(`❌ Template directory not found: ${resolvedPath}`));
            console.log(chalk.yellow('Tip: Run --init-templates to create sample templates'));
            process.exit(1);
        }
        setTemplateDir(resolvedPath);
        console.log(chalk.cyan(`📄 Using custom templates from: ${resolvedPath}`));

        // Show which templates are being used
        const loader = getTemplateLoader();
        const info = loader.getTemplateInfo();
        if (info.customTemplates.length > 0) {
            console.log(chalk.gray(`   Custom: ${info.customTemplates.join(', ')}`));
        }
    }
    ...
    try {
        ...
        // Generate user stats for contributor chart
        const { generateUserStats, getLinesOfCode } = require('./utils');
        const userStats = generateUserStats(commits);

        // Get LOC details with language breakdown
        let locDetails = null;
        try {
            locDetails = getLinesOfCode(process.cwd(), true);
        } catch (e) {
            // Ignore if sloc fails
        }
