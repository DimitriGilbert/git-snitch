...
function showHelp() {
    ...
    console.log(chalk.gray('  1. Run: git-moar scattered --init-templates'));
    console.log(chalk.gray('  2. Edit files in ./templates/ directory'));
    console.log(chalk.gray('  3. Run: git-moar scattered --template-dir ./templates'));
    console.log('');
    ...
    console.log(chalk.gray('  git-moar scattered --template-dir ./my-templates -o branded-report.html'));
    console.log('');
}
...
function generateReport(data) {
    ...
    // Aggregate users across all projects for overview
    const allUsers = {};
    data.projects.forEach(p => {
        p.users.forEach(u => {
            if (!allUsers[u.email]) {
                ...
                allUsers[u.email].totalCommits += u.totalCommits;
                allUsers[u.email].totalAdditions += u.totalAdditions;
                allUsers[u.email].totalDeletions += u.totalDeletions;
            }
        });
    });
    const aggregatedUsers = Object.values(allUsers).sort((a, b) => b.totalCommits - a.totalCommits);
    const allCommits = data.projects.flatMap(p => p.commits);
    ...
    const overviewTab = `
        <div id="overview" class="tab-content">
            ...
            ${config.chartsEnabled() && aggregatedUsers.length > 1 ? `
            <div class="section">
                <h2>👥 Cross-Project Contributors</h2>
                <div class="charts-grid">
                    ${charts.generateUserActivityTimeline(allCommits, aggregatedUsers, 'overview-user-timeline')}
                    ${charts.generateUserContributionsComparison(aggregatedUsers, 'overview-user-contrib')}
                </div>
                <div class="charts-grid">
                    ${charts.generateUserCommitSizeComparison(allCommits, aggregatedUsers, 'overview-user-size')}
                    ${charts.generateUserPeakActivityTimes(allCommits, aggregatedUsers, 'overview-user-peaks')}
                </div>
            </div>
            ` : ''}
        ...
        <div id="project-${pIndex}" class="tab-content">
            <div class="project-card">
                 ...
                 <div id="project-${pIndex}-charts" class="tab-content sub-tab-content">
                    ...
                    ${p.users && p.users.length > 1 ? `
                    <h3 style="margin-top: 30px; color: #f1f5f9;">👥 Contributor Insights</h3>
                    <div class="charts-grid">
                        ${charts.generateUserActivityTimeline(p.commits, p.users, 'project-' + pIndex + '-user-timeline')}
                        ${charts.generateUserContributionsComparison(p.users, 'project-' + pIndex + '-user-contrib')}
                    </div>
                    <div class="charts-grid">
                        ${charts.generateUserCommitSizeComparison(p.commits, p.users, 'project-' + pIndex + '-user-size')}
                        ${charts.generateUserPeakActivityTimes(p.commits, p.users, 'project-' + pIndex + '-user-peaks')}
                    </div>
                    ` : ''}
                 </div>
                 ` : ''}
...
function run(args) {
    ...
    if (options.initTemplates) {
        ...
        console.log(chalk.gray(`  2. Run: git-moar scattered --template-dir ${templatesDir}`));
        return;
    }
