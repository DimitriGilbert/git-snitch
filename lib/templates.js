...
const { TemplateLoader, defaultLoader } = require('./template-loader');

// Current template loader instance (can be swapped for custom templates)
let currentLoader = defaultLoader;

/**
 * Set a custom template directory
 * @param {string} templateDir - Path to custom template directory
 */
function setTemplateDir(templateDir) {
    if (templateDir) {
        currentLoader = new TemplateLoader(templateDir);
    } else {
 ...
 * Supports custom templates via template loader
 */
function generateHTMLDocument(title, content, customStyles = '', additionalScripts = '') {
    // Check if we have a custom document template
    if (currentLoader.hasCustomTemplate('document')) {
        const customTemplateStyles = currentLoader.loadStyles();
        const customTemplateScript = currentLoader.loadScript();

        return currentLoader.render('document', {
            title,
            styles: commonStyles + '\n' + customStyles + '\n' + customTemplateStyles,
            script: commonScript + '\n' + customTemplateScript,
            content,
            additionalScripts
        });
    }
...
function generateHeader(title, subtitle) {
    if (currentLoader.hasCustomTemplate('header')) {
        return currentLoader.render('header', { title, subtitle });
    }
...
function generateStatsGrid(stats) {
    // Generate stat cards
    let cards;
    if (currentLoader.hasCustomTemplate('statCard')) {
        cards = stats.map(stat => currentLoader.render('statCard', {
            value: stat.value,
            label: stat.label,
            class: stat.class || ''
        })).join('\n');
    ...
    if (currentLoader.hasCustomTemplate('statsGrid')) {
        return currentLoader.render('statsGrid', { cards });
    }
